...
void audio_callback_vod(void *userdata, Uint8 *stream, int len) //音频回调函数，SDL_OpenAudio使用后会无限循环使用该函数，直到使用SDL_closeAudio，如果不close则SDL_Quit无限阻塞
{
	...
	while(read_this_buf->bufsize<=0)Sleep(too_fast_wait);//如果该缓冲块为空，一直等待
	...
	if((audio_pace+7)>decode_audio_pace)Sleep(2);//如果缓冲不够了，稍微做出看不出的停顿
...
void process_video_function_vod()
{
	...
	if(pVideoCodecCtx_vod->width>my_width)set_width=my_width;else set_width=pVideoCodecCtx_vod->width;//设置sdl屏幕长宽
	...
	while(true)
    {
		...
		while(video_pace==decode_video_pace&&!write_video_buff_over){SDL_PollEvent(&event);Sleep(100);}//如果还没接受到足够多的数据，进行等待
    ...
    SDL_DestroyWindow(pSDLWindow);cout<<"图片处理结束\n";// 销毁窗口
}
