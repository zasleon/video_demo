...
{
	if(!first_wait){Sleep(master_delay);first_wait=true;audio_pace=0;}
	...
	if((video_pace+2)>decode_video_pace&&!write_video_buff_over)Sleep(4000);//如果缓冲很不够了，强制停顿
	...
	if((read_count==bufnumber/4||read_count==(bufnumber*3/4))&&read_this_buf->read_buf_index<len){next_write=true;}//可以让写服务继续写了
	...
	else
	{
		...
		if(write_audio_buff_over&&read_this_buf->write_buf_index==0){audio_output_over=true;cout<<"音频播放结束了\n";}//如果音频没帧了且下一块内存内容为空，宣布输出彻底结束output_over=true
...
void process_video_function_vod()
{
    ...
    numBytes = avpicture_get_size(AV_PIX_FMT_YUV420P,pVideoCodecCtx_live->width,pVideoCodecCtx_live->height);
    outBuffer = (unsigned char *)av_malloc(numBytes);
    avpicture_fill((AVPicture *)pAVFrameRGB32,outBuffer,AV_PIX_FMT_YUV420P,pVideoCodecCtx_live->width,pVideoCodecCtx_live->height);//pAVFrame32的data指针指向了outBuffer
	...
	if(pVideoCodecCtx_live->width>my_width)set_width=my_width;else set_width=pVideoCodecCtx_live->width;//设置sdl屏幕长宽
	if(pVideoCodecCtx_live->height>my_height) set_height=my_height;else set_height=pVideoCodecCtx_live->height;
	...
	while(true)
    {
		...
		while(video_pace+2>decode_video_pace&&!write_video_buff_over){Sleep(4000);}//如果还没接受到足够多的数据，进行等待

		avcodec_send_packet(pVideoCodecCtx_live, pAVPacket[read_v_count]);
		AVFrame *pAVFrame=av_frame_alloc();			// ffmpeg单帧格式
		while(!avcodec_receive_frame(pVideoCodecCtx_live, pAVFrame))
        {
...
void show_moive_vod()//点播
{
	...
	video_output_over=false;write_video_buff_over=false;//初始化，都没播完
	...
	else
	{
		...
		if(avcodec_open2(pVideoCodecCtx_live, vCodec, &pAVDictionary)){cout << "avcodec_open2 failed\n";return;}
	...
	//------------------------------------------------音频配置开始-----------------------------------------------
	next_write=false;
	audio_pace=0;decode_audio_pace=0;//初始速度默认为0
	...
	if(videoIndex>=0)CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)process_video_function_vod, NULL, NULL, NULL);//如果有视频，开始视频线程部署并播放
	...
	if(cut==0)//移动到指定时间点，跳转功能
	{ 
		cut++;
		cout<<"强切\n";
		int set_time=8000;//第1秒
    ...
    if(pVideoCodecCtx_live){avcodec_close(pVideoCodecCtx_live);pVideoCodecCtx_live = 0;}
